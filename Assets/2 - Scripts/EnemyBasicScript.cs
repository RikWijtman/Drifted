...
public class EnemyBasicScript : MonoBehaviour
{
    ...
    private LayerMask obstacleMask; // Voeg dit veld toe bovenaan bij de andere velden
    ...
    void Start()
    {
        ...
        obstacleMask = LayerMask.GetMask("Block"); // Zorg dat je obstakels in deze layer zet!
    }
        ...
        if (move.moveType == Moves.AttackTypes.Ranged)
        {
            if (move.projectile != null)
            {
                ...
                if (projectileScript != null)
                {
                    ...
                    projectileScript.sourceMove = move;
                }
            }
            else
            {
    ...
    private void Roaming()
    {
        ...
        if (roamingTarget == null || Vector2.Distance(transform.position, roamingTarget.Value) < patrolPointThreshold)
        {
            ...
            for (int i = 0; i < maxTries; i++)
            {
                ...
                Vector2 candidate = startPosition + randomCircle;

                bool blocked = false;
                if (!enemyType.isAerial)
                {
                    Collider2D hit = Physics2D.OverlapPoint(candidate, obstacleMask);
                    blocked = hit != null;
                }

                if (!blocked)
                {
                    roamingTarget = candidate;
                    break;
                }
            }
            // Als na maxTries geen geldige plek is gevonden, blijf staan
