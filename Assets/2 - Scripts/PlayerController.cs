...
public class PlayerController : MonoBehaviour
{
    ...
    private bool waitingForWakeInput = true;
    ...
    void Update()
    {
        ...
        {
            // Wacht op een willekeurige input
            if (Input.anyKeyDown || Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1))
            {
                waitingForWakeInput = false;
                if (animator != null)
                {
                    animator.SetTrigger("WakeUp");
                }
                StartCoroutine(WakeUpAfterAnimation());
            }
            return;
        }
    ...
    private IEnumerator WakeUpAfterAnimation()
    {
        // Wacht tot de animatie klaar is (pas evt. de tijd aan naar je animatie)
        yield return new WaitForSeconds(1f); // Bijvoorbeeld 1.2 seconden
