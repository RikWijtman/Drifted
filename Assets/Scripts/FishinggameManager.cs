...
public class FishingMinigameManager : MonoBehaviour
{
    ...
    private void MoveTarget()
    {
        ...
        Vector3 newPos = targetCircle.anchoredPosition + (Vector2)currentDirection * targetMoveSpeed * Time.deltaTime;

        // Controleer of de vis tegen een rand botst
        if (newPos.x <= -panelBounds.x + targetCircle.rect.width / 2 || newPos.x >= panelBounds.x - targetCircle.rect.width / 2)
        {
            currentDirection.x *= -1; // Keer horizontale richting om
            newPos.x = Mathf.Clamp(newPos.x, -panelBounds.x, panelBounds.x);
        }

        if (newPos.y <= -panelBounds.y + targetCircle.rect.height / 2 || newPos.y >= panelBounds.y - targetCircle.rect.height / 2)
        {
        ...
        // Beweeg de vis
        targetCircle.anchoredPosition = newPos;
    }
    ...
    private void UpdateRingProgress()
    {
        ...
        float distance = Vector2.Distance(hookIndicator.anchoredPosition, targetCircle.anchoredPosition);
